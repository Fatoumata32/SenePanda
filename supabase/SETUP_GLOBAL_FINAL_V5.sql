-- ========================================
-- SETUP GLOBAL FINAL V5 - AVEC FOREIGN KEY
-- ========================================
-- Ce script corrige TOUT automatiquement
-- Ajoute la relation entre user_subscriptions et subscription_plans
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE '🚀 SETUP GLOBAL V5 - SYNC AUTO + FK';
  RAISE NOTICE '========================================';
  RAISE NOTICE '';
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '⚠️  Colonne currency existe déjà';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⚠️  Colonne duration_days existe déjà';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⚠️  Colonne max_products existe déjà';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⚠️  Colonne max_shops existe déjà';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⚠️  Colonne features existe déjà';
  END IF;
  ...
  ELSE
    RAISE NOTICE '⚠️  Colonne is_active existe déjà';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'subscription_plans' AND column_name = 'plan_type') THEN
    ALTER TABLE subscription_plans ADD COLUMN plan_type TEXT DEFAULT 'standard';
    RAISE NOTICE '✅ Colonne plan_type ajoutée';
  ELSE
    RAISE NOTICE '⚠️  Colonne plan_type existe déjà';
  END IF;
END $$;

-- 1.3 Mettre à jour les plans existants (AVEC plan_type et JSON valide)
DO $$
DECLARE
  starter_features JSONB := '["Jusqua 10 produits", "1 boutique", "Support par email"]'::jsonb;
  premium_features JSONB := '["Produits illimites", "3 boutiques", "Support prioritaire"]'::jsonb;
  business_features JSONB := '["Tout Premium", "10 boutiques", "Support 24/7"]'::jsonb;
BEGIN
  -- Plan Starter
  IF EXISTS (SELECT 1 FROM subscription_plans WHERE LOWER(name) = 'starter') THEN
    ...
    SET
      ...
      features = COALESCE(features, starter_features),
      is_active = COALESCE(is_active, true)
    WHERE LOWER(name) = 'starter';
    RAISE NOTICE '✅ Plan Starter mis à jour';
  ELSE
    INSERT INTO subscription_plans (name, description, plan_type, price, currency, duration_days, max_products, max_shops, features, is_active)
    VALUES ('Starter', 'Plan de démarrage', 'basic', 5000.00, 'FCFA', 30, 10, 1, starter_features, true);
  ...
  IF EXISTS (SELECT 1 FROM subscription_plans WHERE LOWER(name) = 'premium') THEN
    ...
    SET
      ...
      features = COALESCE(features, premium_features),
      is_active = COALESCE(is_active, true)
    WHERE LOWER(name) = 'premium';
    RAISE NOTICE '✅ Plan Premium mis à jour (produits illimités)';
  ELSE
    INSERT INTO subscription_plans (name, description, plan_type, price, currency, duration_days, max_products, max_shops, features, is_active)
    VALUES ('Premium', 'Plan complet', 'premium', 15000.00, 'FCFA', 30, NULL, 3, premium_features, true);
  ...
  IF EXISTS (SELECT 1 FROM subscription_plans WHERE LOWER(name) = 'business') THEN
    UPDATE subscription_plans
    SET
      plan_type = COALESCE(plan_type, 'business'),
      price = COALESCE(price, 50000.00),
      currency = COALESCE(currency, 'FCFA'),
      duration_days = COALESCE(duration_days, 30),
      max_products = NULL, -- NULL = illimité
      max_shops = COALESCE(max_shops, 10),
      features = COALESCE(features, business_features),
      is_active = COALESCE(is_active, true)
    WHERE LOWER(name) = 'business';
    RAISE NOTICE '✅ Plan Business mis à jour (produits illimités)';
  ELSE
    INSERT INTO subscription_plans (name, description, plan_type, price, currency, duration_days, max_products, max_shops, features, is_active)
    VALUES ('Business', 'Solution professionnelle', 'business', 50000.00, 'FCFA', 30, NULL, 10, business_features, true);
...
-- ========================================
-- PARTIE 2.5 : AJOUTER FOREIGN KEY (NOUVEAU!)
-- ========================================
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Foreign key plan_id → subscription_plans ajoutée';
  ELSE
    RAISE NOTICE '⚠️  Foreign key plan_id existe déjà';
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '  ✓ Foreign Key : %', CASE WHEN v_has_fk THEN 'Configurée' ELSE 'Manquante' END;
  RAISE NOTICE '  ✓ Realtime : Activé';
  RAISE NOTICE '  ✓ RLS : Configuré';
  RAISE NOTICE '  ✓ Index : Créés';
  RAISE NOTICE '  ✓ plan_type : Inclus';
  RAISE NOTICE '  ✓ JSON : Valide';
  RAISE NOTICE '';
...
SELECT
  name,
  COALESCE(plan_type, 'N/A') as type,
